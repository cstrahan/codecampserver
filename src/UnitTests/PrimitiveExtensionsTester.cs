...
namespace CodeCampServer.UnitTests
{
	...
	public class PrimitiveExtensionsTester
	{
		...
		{
			"1/1/2008".ToNullableDate().Value.ShouldEqual(new DateTime(2008, 1, 1));
			"as".ToNullableDate().HasValue.ShouldBeFalse();
		}

		[Test]
		public void Should_convert_pascal_cased_text_to_separated_words()
		{
			"One".ToSeparatedWords().ShouldEqual("One");
			"OneTwo".ToSeparatedWords().ShouldEqual("One Two");
			"OneAndTwo".ToSeparatedWords().ShouldEqual("One And Two");
			"OneAndATwoAndAThree".ToSeparatedWords().ShouldEqual("One And A Two And A Three");
			"One2Three".ToSeparatedWords().ShouldEqual("One2 Three");
		}
