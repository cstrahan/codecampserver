	...
	public class MappingEngine : IMappingEngine
	{
		...
		private object CreateMappedObject(ResolutionContext context)
		{
			...
			foreach (PropertyMap propertyMap in context.SourceValueTypeMap.GetPropertyMaps())
			{
				...
				if (resolver != null)
				{
					object inputValueToResolve = context.SourceValue;
				...
				TypeMap memberTypeMap = Configuration.FindTypeMapFor(modelMemberType, propertyMap.DestinationProperty.PropertyType);

				ResolutionContext newContext = context.CreateMemberContext(memberTypeMap, modelMemberValue, modelMemberType,
				                                                           propertyMap);
		...
		private object CreateArrayObject(ResolutionContext context)
		{
			...
			foreach (object item in enumerableValue)
			{
				...
				ResolutionContext newContext = context.CreateElementContext(derivedTypeMap, item, targetSourceType,
				                                                            targetDestinationType, i);
		...
		private static Type GetElementType(Type enumerableType)
		{
			...
			throw new ArgumentException(string.Format("Unable to find the element type for type '{0}'.", enumerableType),
			                            "enumerableType");
		}
