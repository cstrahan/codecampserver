...
namespace CodeCampServer.Infrastructure.AutoMap
{
	public class TypeMap
	{
		...
		{
			IEnumerable<string> autoMappedProperties = _propertyMaps.Where(
				pm => pm.HasSourceMember() || pm.HasCustomValueResolver() || pm.Ignored)
		...
		{
			PropertyMap propertyMap = _propertyMaps.FirstOrDefault(pm => pm.DestinationProperty.Name.Equals(dtoProperty.Name));
			if (propertyMap == null)
			{
