...
namespace RegressionTests.Web
{
    ...
    public class WebTest : CSSIdentifiers
    {
        private static readonly LocalDataStoreSlot DATA_SLOT = Thread.AllocateDataSlot();
        ...
        public virtual void TearDown()
        {
            try
            {
                bool testError = TestContext.CurrentContext.Outcome.Status == TestStatus.Failed;
                ...
                if (testError || serverError)
                {
                    ...
                    Assert.IsFalse(serverError, "An error left the system in a failing state.\n" + ErrorContext());
                }
                LogoutIfLoggedIn();
                ie.GoTo("about:blank");
            }
        ...
        private void Snapshot(string caption, TestLogStreamWriter logStreamWriter)
        {
            using (logStreamWriter.BeginSection(caption))
            {
                ...
                logStreamWriter.EmbedImage(caption + ".png",
                                           new CaptureWebPage(ie).CaptureWebPageImage(false, false, 100));
            }
        }
        ...
        protected void Follow(string transition, IDictionary<string, object> withParams, int countDown)
        {
        ...
        protected void Follow(string transition, IDictionary<string, object> withParams, Until until)
        {
            ExecutionSteps.Log.WriteLine("Navigate to '{0}'", transition);
            if (withParams != null)
            {
                ExecutionSteps.Log.WriteLine("Perform '{0}' action with the following parameters:", transition);
        ...
        protected void Follow(string transition, IDictionary<string, object> withParams, Until until, int countDown)
        {
            string after = null;
            if (until != null)
                after = " looking for: " + until.s;

            Assert.IsTrue(countDown > 0, "Didn't match transition:" + transition + after + "\n" + ErrorContext());

            Element elem = LocateLink(transition);

            Assert.IsNotNull(elem, "Can't find element for transition: " + transition + "\n" + ErrorContext());
        ...
        protected static string PropertyToCSS<TDTO>(Expression<Func<TDTO, object>> memberExpr)
        {
