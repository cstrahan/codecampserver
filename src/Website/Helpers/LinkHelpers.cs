...
namespace CodeCampServer.Website.Helpers
{
	public static class LinkHelpers
	{
		...
		/// <param name="classIfActive">the css class to set if this is an active link</param>
		/// <returns>an html A tag</returns>
		public static string NavLink(this ViewMasterPage masterPage, string text, string controller, string action,
		                             string classIfActive)
		{
		...
		{
			var action = (string) RouteHelpers.GetPropertyHash(values)["action"];
